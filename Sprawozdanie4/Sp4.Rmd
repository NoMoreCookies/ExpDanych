...
ggplot(pca_df, aes(x = PC1, y = PC2, color = Klaster, shape = Klasa)) +
  ...
  labs(title = "PAM na danych Glass: Kolory = Klaster, Kształty = Klasa",
       x = "PC1", y = "PC2") +
  theme_minimal() +
  scale_color_manual(values = RColorBrewer::brewer.pal(7, "Set1")) +  # 6 kolorów
...
Z drugiej strony, zaobserwowano **niską jednorodność klas pod względem etykiet rzeczywistych** – obiekty należące do różnych klas (oznaczone różnymi kolorami) **mieszają się wewnątrz tych samych skupień**. Szczególnie wyraźne jest to w przypadku **klas brązowej i pomarańczowej** oraz **zielonej i niebieskiej**, które często współwystępują w tych samych obszarach przestrzeni klastrów.

```{r Zgodnosci}
# Tabela zgodności
tabela_zgodnosci <- table(Klaster = pam_model$clustering, Klasa = Y)

# Maksimum z każdej kolumny – najliczniejsza klasa w danym klastrze
najwiecej_na_klaster <- apply(tabela_zgodnosci, 1, max)

# Obliczenie sumy poprawnych przypisań
poprawne <- sum(najwiecej_na_klaster)

# Całkowita liczba obserwacji
n <- sum(tabela_zgodnosci)

# Procent poprawnych przypisań
dokladnosc <- poprawne / n * 100
...
W wyniku analizy zgodności przypisań klastrów do klas rzeczywistych, obliczono tzw. **wskaźnik zgodności (purity)**. Niestety, uzyskana wartość wyniosła jedynie **51,4%**, co należy uznać za **niski poziom dopasowania**.
...
Przyczyną wystąpienia tego efektu w analizowanych danych jest **duży rozrzut obserwacji** oraz **obecność wartości odstających**. Te same czynniki wpłynęły również negatywnie (leczw o wiele mniejszym stopniu) na wyniki uzyskane za pomocą metody **complete linkage**, w której efekt łańcuchowy również jest widoczny, choć w nieco łagodniejszej formie.

W przypadku metody **average linkage**, zjawisko łańcuchowe **również występuje**, jednak jego **intensywność jest znacząco mniejsza**. Klastery są **bardziej zwarte i lepiej odseparowane**, co przekłada się na **większą równowagę w podziale danych** oraz **lepszą zgodność z rzeczywistym podziałem klas**.

## c) Ocena jakości grupowania. Wybór optymalnej liczby skupień i porównanie metod.

### Wskaźniki wewnętrzne

**W celu dokładniejszego porównania działania poszczególnych algorytmów, ocena została przeprowadzona na oryginalnych (niestandaryzowanych) danych.** Pozwoliło to zachować rzeczywiste różnice w skali poszczególnych zmiennych, co może mieć istotny wpływ na sposób formowania klastrów przez różne metody grupowania.
...
# Zakładamy, że dane Glass są przygotowane jako X_scaled (bez kolumny klasy, standaryzowane)
df_glass_ready <- Glass[,-10]  
...
for (k in K.range) {
  # PAM
  pam.model <- pam(df_glass_ready, k)
  ...
  for (method in agnes.methods) {
    agnes.model <- agnes(df_glass_ready, method = method)
    clust <- cutree(as.hclust(agnes.model), k = k)
    sil.agnes <- mean(silhouette(clust, dist(df_glass_ready))[, 3])
...
ggplot(results_glass, aes(x = K, y = Silhouette, color = Method)) +
  geom_line(size = 1.2) +
  geom_point(size = 3) +
  geom_point(data = best_glass, aes(x = K, y = Silhouette), color = "black", size = 4, shape = 8)  +
...
true_labels_matched[true_labels_matched > 4] <- true_labels_matched[true_labels_matched >4] - 1 
...
# Przykład: glass to Twoja ramka danych, bierzemy tylko kolumny numeryczne (jeśli tam są też inne)
# Załóżmy, że glass ma cechy w kolumnach 1:9 (dostosuj do swojego zbioru)
glass_features <- Glass[, 1:9]

# Klasteryzacja aglomeracyjna z metodą complete linkage
agnes_complete <- agnes(glass_features, method = "complete")
...
glass_with_clusters <- data.frame(glass_features, Cluster = as.factor(clusters))
...
# Załóżmy, że glass to Twoja ramka danych, bierzemy kolumny 1:9
glass_features <- Glass[, 1:9]
...
# Dodajemy klastry do danych
glass_with_clusters <- data.frame(glass_features, Cluster = as.factor(clusters))

# Obliczamy średnie wartości cech w klastrach
cluster_means <- glass_with_clusters %>%
  group_by(Cluster) %>%
  summarise(across(everything(), ~ round(mean(.x, na.rm = TRUE), 2)))
