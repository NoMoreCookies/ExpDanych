...
for (pair in pairs) {
  ...
  complete_data <- data_numeric[complete.cases(data_numeric[, c(var1, var2)]), ]
...
top_correlations <- head(results, 10)

plot_data <- top_correlations
plot_data$Pair <- paste(plot_data$Variable1, "&", plot_data$Variable2)
plot_data$Pair <- factor(plot_data$Pair, levels = rev(plot_data$Pair))
plot_data$P_Value_Format <- format.pval(plot_data$P_Value, digits = 3)
...
ggplot(plot_data, aes(x = Correlation, y = Pair, fill = Correlation)) +
  ...
  labs(
    title = paste(nrow(plot_data), "najsilniejszych istotnych korelacji"), 
  ...
  geom_text(
    aes(label = sprintf("r = %.3f (p = %s)", Correlation, P_Value_Format)), 
...
most_common <- names(sort(table(data$Embarked), decreasing = TRUE))[1]
...
**Ocena jakości odwzorowania – Diagram Shepherda**
...
**Wniosek:** Transformacja MDS zachowała strukturę danych na akceptowalnym poziomie. Diagram Shepherda potwierdza, że skalowanie wielowymiarowe wiernie odtworzyło relacje między punktami, szczególnie w przypadku najbliższych sąsiadów.  
